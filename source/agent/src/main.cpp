...
_Bool GetConnectionInfoFromADUConfigFile(ADUC_ConnectionInfo* info)
{
    ...
    if (0/*!ReadDelimitedValueFromFile(
...
_Bool StartupAgent(const ADUC_LaunchArguments* launchArgs)
{
    ...
    {
        const int result = 0;//deliveryoptimization_set_iot_connection_string(info.connectionString);
        if (result != 0)
        {
...
int main()
{
    ...
    #if 0
    ret = ParseLaunchArguments(0/*argc*/, NULL/*argv*/, &launchArgs);
    if (ret != 0)
    {
        return ret;
    }
    ...
    MBEDBL_UCP_PrepareWrite(&ucp_ctx, 0, version, size_file);
    MBEDBL_UCP_Write(&ucp_ctx, (uint8_t *) buf_ota, &fragment_size);
    MBEDBL_UCP_FinalizeWrite(&ucp_ctx);
