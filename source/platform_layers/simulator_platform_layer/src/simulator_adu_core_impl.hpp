...
private:
        ...
        const ADUC_DownloadInfo* info)
    {
        //try
        {
            ...
            worker.detach();
            #else
            static_cast<SimulatorPlatformLayer*>(token)->Download(workflowId, updateType, info);
            #endif
        ...
        const ADUC_InstallInfo* info)
    {
        //try
        {
            ...
            worker.detach();
            #else
            static_cast<SimulatorPlatformLayer*>(token)->Install(workflowId, info);
            #endif
            // Indicate that we've spun off a thread to do the actual work.
        ...
        const ADUC_ApplyInfo* info)
    {
        //try
        {
            ...
            worker.detach();
            #else
            static_cast<SimulatorPlatformLayer*>(token)->Apply(workflowId, info);
            #endif
            // Indicate that we've spun off a thread to do the actual work.
    ...
    static void CancelCallback(ADUC_Token token, const char* workflowId)
    {
        ...
        #else
        static_cast<SimulatorPlatformLayer*>(token)->Cancel(workflowId);
        #endif
    }
